...
import { getPanelToken } from './auth-token';
...
myAxios.wsExec = function (name, podName, shell, onMessage, onSuccess, onError) {
    const ws = new WebSocket(`${newApiSuffix}/exec?podName=${podName}&containerName=${name}&tty=false&namespace=default&command=sh&command=-c&command=${encodeURIComponent(shell)}&api-token=${getPanelToken()}`)
