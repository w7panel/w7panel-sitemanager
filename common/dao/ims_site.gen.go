...
func newSite(db *gorm.DB, opts ...gen.DOOption) site {
	...
	_site.Domain = field.NewString(tableName, "domain")
	...
	_site.SiteSetting = siteHasManySiteSetting{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("SiteSetting", "entity.SiteSetting"),
	}

	_site.Environment = siteEnvironment{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Environment", "entity.Environment"),
	}
...
type site struct {
	...
	SiteSetting   siteHasManySiteSetting
...
type siteHasManySiteSetting struct {
	db *gorm.DB

	field.RelationField
}

func (a siteHasManySiteSetting) Where(conds ...field.Expr) *siteHasManySiteSetting {
	if len(conds) == 0 {
		return &a
	}
...
func (a siteHasManySiteSetting) WithContext(ctx context.Context) *siteHasManySiteSetting {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a siteHasManySiteSetting) Session(session *gorm.Session) *siteHasManySiteSetting {
	a.db = a.db.Session(session)
	return &a
}

func (a siteHasManySiteSetting) Model(m *entity.Site) *siteHasManySiteSettingTx {
	return &siteHasManySiteSettingTx{a.db.Model(m).Association(a.Name())}
}

type siteHasManySiteSettingTx struct{ tx *gorm.Association }

func (a siteHasManySiteSettingTx) Find() (result []*entity.SiteSetting, err error) {
	return result, a.tx.Find(&result)
}

func (a siteHasManySiteSettingTx) Append(values ...*entity.SiteSetting) (err error) {
...
func (a siteHasManySiteSettingTx) Replace(values ...*entity.SiteSetting) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a siteHasManySiteSettingTx) Delete(values ...*entity.SiteSetting) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a siteHasManySiteSettingTx) Clear() error {
	return a.tx.Clear()
}

func (a siteHasManySiteSettingTx) Count() int64 {
	return a.tx.Count()
}
...
func (a siteEnvironment) Where(conds ...field.Expr) *siteEnvironment {
	if len(conds) == 0 {
		return &a
	}
...
func (a siteEnvironment) WithContext(ctx context.Context) *siteEnvironment {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a siteEnvironment) Session(session *gorm.Session) *siteEnvironment {
	a.db = a.db.Session(session)
	return &a
}

func (a siteEnvironment) Model(m *entity.Site) *siteEnvironmentTx {
	return &siteEnvironmentTx{a.db.Model(m).Association(a.Name())}
}

type siteEnvironmentTx struct{ tx *gorm.Association }

func (a siteEnvironmentTx) Find() (result *entity.Environment, err error) {
	return result, a.tx.Find(&result)
}

func (a siteEnvironmentTx) Append(values ...*entity.Environment) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a siteEnvironmentTx) Replace(values ...*entity.Environment) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a siteEnvironmentTx) Delete(values ...*entity.Environment) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a siteEnvironmentTx) Clear() error {
	return a.tx.Clear()
}

func (a siteEnvironmentTx) Count() int64 {
	return a.tx.Count()
}
